...
import (
	...
	"github.com/nirabyte/todo/internal/animations"
...
func (m *models.Model) View() string {
	...
	sortStr := "Off"
	if m.SortMode == models.SortTodoFirst {
		sortStr = "Todo"
	}
	if m.SortMode == models.SortDoneFirst {
		sortStr = "Done"
	}
...
func (m *models.Model) viewList(t themes.Theme) string {
	if len(m.Tasks) == 0 && m.State != models.StateCreating {
	...
	for i := 0; i < count; i++ {
		...
		isEditingThis := (m.State == models.StateEditing && i == m.Cursor)
		isCreatingThis := (m.State == models.StateCreating && i == creatingIndex)
		isSettingTime := (m.State == models.StateSettingTime && i == m.Cursor)
		...
		if isEditingThis || isCreatingThis {
			...
			var rawTitle string
			if task.IsDeleting {
				rawTitle = animations.RenderDeleteAnim(task.Title, t)
			} else if task.IsAnimatingCheck {
				rawTitle = animations.RenderCheckAnim(task, t)
			} else if task.Done {
